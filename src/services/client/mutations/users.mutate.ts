...
export async function followOrUnfollowPlaylist(playlist_id: string, type: 'follow' | 'unfollow'): Promise<void> {
  try {
    if(type === 'unfollow') return await spotify.currentUser.playlists.unfollow(playlist_id)
...
export async function followOrUnfollowArtistsOrUsers(ids: string[], type: 'artist' | 'user', followOrUnfollow: 'follow' | 'unfollow') {
  try {
    if(followOrUnfollow === 'unfollow') return await spotify.currentUser.followArtistsOrUsers(ids, type)
